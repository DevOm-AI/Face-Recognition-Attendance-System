...
class Attendance:
    ...
    def __init__(self,root):
        ...
        title_lb1.place(x=200,y=50,width=1000,height=60)

        #========================Section Creating==================================

        # Creating Frame 
        main_frame = Frame(bg_img,bd=2,bg="white") #bd mean border 
        main_frame.place(x=0,y=125,width=1355,height=510)
        ...
        #create table 
        self.attendanceReport = ttk.Treeview(table_frame,column=("ID","Roll","Name","Time","Date","Status"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport.heading("Status",text="Attend-status")
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(user='root', password='0417',host='localhost',database='face_recognizer',port=3306)
                    mycursor = conn.cursor()
                    mycursor.execute("update stdattendance set ID=%s,Roll=%s,Name=%s,Time=%s,Date=%s,Status=%s where ID=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
