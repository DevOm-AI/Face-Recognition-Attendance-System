...
# Testing Connection 
"""
conn = mysql.connector.connect(username='root', password='0417',host='localhost',database='face_recognizer',port=3306)
cursor = conn.cursor()
...
class Student:
    ...
    # This part is image labels setting start 
        ...
        self.photoimg = ImageTk.PhotoImage(Image.open("D:\\Face Recognition Attendance System Project\\Images_GUI\\banner.jpg"))
        ...
        # Back Button Code Snippet
        back_button = Button(f_lb1, text="Back To Main Menu", command=go_to_main, font=("verdana", 12, "bold"), bg="navyblue", fg="white")
        back_button.place(x=550, y=40)
...
# ==================Function Decleration==============================
    ...
    def update_data(self):
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            ...
            try:
                Update = messagebox.askyesno("Update", "Do You Really Want To Update?", parent=self.root)
                if Update > 0:
                    conn = mysql.connector.connect(username='root', password='0417',host='localhost',database='face_recognizer',port=3306)
                    mycursor = conn.cursor()
                    print(self.var_dob.get())
                    mycursor.execute("update student set `dep`=%s, `course`=%s, `year`=%s, `sem`=%s, `name`=%s, `div`=%s, `roll`=%s, `gender`=%s, `dob`=%s, `email`=%s, `mob`=%s, `address`=%s, `teacher`=%s, `photo`=%s where `id`=%s",(
                                                                                                                                                                                                    self.var_dep.get(),
                                                                                                                                                                                                    self.var_course.get(),
                                                                                                                                                                                                    self.var_year.get(),
                                                                                                                                                                                                    self.var_semester.get(),
    ...
    def search_data(self):
        if self.var_search.get()=="" or self.var_searchTX.get()=="Select":
            ...
            try:
                ...
                sql = "SELECT ID,Name,Dep,Course,Year,Sem,Div,Gender,DOB,Mob,Address,Roll,Email,Teacher,Photo FROM student where Roll-No='" +str(self.var_search.get()) + "'" 
            ...
            try:
                ...
                while True:
                    ...
                    if face_croped(my_frame) is not None:
                        ...
                        file_path="Image_Dataset/img."+str(id)+"."+str(img_id)+".jpg"
